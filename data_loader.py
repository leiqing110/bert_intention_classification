...
class CPWSDataset(ListDataset):
    ...
    def load_data(filename):
        ...
        return data
# 加载custom数据
class CustomDataset(ListDataset):
    @staticmethod
    def load_data(filename):
        data = []
        with open(filename, encoding='utf-8') as f:
...
if __name__ == "__main__":
    ...
    max_len = 512
    tokenizer = BertTokenizer.from_pretrained('../model_hub/roberta_base_model')
    train_dataset = CustomDataset(file_path='data/custom/train_data.txt')
    print(train_dataset[0])

    with open('data/custom/labels.txt', 'r', encoding="utf-8") as fp:
