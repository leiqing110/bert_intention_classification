...
import models
import utils
from data_loader import CNEWSDataset, Collate, CPWSDataset, CustomDataset
...
class Trainer:
    ...
    def get_classification_report(self, outputs, targets, labels):
        # pdb.set_trace()
        report = classification_report(targets, outputs, labels=range(len(labels)),target_names=labels,digits=4)
        return report
